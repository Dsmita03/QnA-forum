    ...
    const fetchNotifications = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const processedNotifications: Notification[] = rawData
            .map((item, index) => {
              ...
              if (!item || typeof item !== 'object') {
                console.warn(`Invalid notification at index ${index}:`, item);
                return null;
              }

              // Create a valid notification with guaranteed unique ID
              return {
                id: String(item.id || `fallback-${Date.now()}-${index}`),
          ...
          const uniqueNotifications = processedNotifications.map((notification) => {
          ...
          console.log("Processed notifications with unique IDs:", uniqueNotifications);
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100 bg-gradient-to-r from-orange-50 to-amber-50">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={allNotification.every((n) => n.seen) || loading}
