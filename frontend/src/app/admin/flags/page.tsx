...
export default function AdminReviewFlagsPage() {
  ...
  const fetchFlags = async () => {
    try {
      ...
      const pending = response.data.filter((f: FlaggedItem) => f.status === 'pending').length;
      const reviewed = response.data.filter((f: FlaggedItem) => f.status === 'reviewed').length;
      const dismissed = response.data.filter((f: FlaggedItem) => f.status === 'dismissed').length;
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'pending': return 'bg-gradient-to-r from-amber-100 to-orange-200 text-orange-800 border border-orange-300 shadow-md animate-pulse';
      case 'reviewed': return 'bg-gradient-to-r from-orange-500 to-orange-600 text-white border border-orange-600 shadow-md';
      case 'dismissed': return 'bg-gradient-to-r from-orange-200 to-orange-300 text-orange-800 border border-orange-400 shadow-sm';
      default: return 'bg-gradient-to-r from-orange-50 to-orange-100 text-orange-700 border border-orange-200 shadow-sm';
    }
  };
  ...
  const getCategoryBadge = (category: string) => {
    switch (category) {
      case 'spam': return 'bg-gradient-to-r from-orange-300 to-orange-400 text-orange-900 border border-orange-500 shadow-md';
      case 'inappropriate': return 'bg-gradient-to-r from-orange-400 to-red-400 text-white border border-orange-500 shadow-md';
      case 'harassment': return 'bg-gradient-to-r from-orange-500 to-pink-400 text-white border border-orange-600 shadow-md';
      case 'copyright': return 'bg-gradient-to-r from-orange-200 to-amber-300 text-orange-900 border border-orange-400 shadow-md';
      default: return 'bg-gradient-to-r from-orange-100 to-orange-200 text-orange-800 border border-orange-300 shadow-sm';
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-orange-700/80 text-sm font-medium">Moderate and manage reported content across your platform</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-gradient-to-r from-orange-100 to-orange-200 px-4 py-2 rounded-full shadow-lg border border-orange-300">
              ...
              <span className="text-xs font-bold text-orange-800">{filteredFlags.length} reports</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            >
              <Card className={`bg-gradient-to-br ${stat.gradient} text-white shadow-xl border-0 overflow-hidden rounded-2xl`}>
              ...
              {loading ? (
                <div className="flex items-center justify-center py-16">
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-orange-600 text-lg font-semibold">Loading flagged content...</p>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="divide-y divide-orange-100">
                    {paginatedFlags.length === 0 ? (
                      <div className="text-center py-16">
                        ...
                        <h3 className="text-lg font-bold text-orange-800 mb-2">No flagged content found</h3>
                        <p className="text-orange-600 text-sm">All reports have been processed or no reports match your criteria.</p>
                      </div>
                    ) : (
                      <AnimatePresence>
                        {paginatedFlags.map((flag, index) => (
                          <motion.div
                            key={flag.id}
                            ...
                            <div className="col-span-1 flex items-center">
                              <div className={`p-2 rounded-xl shadow-md border ${
                                ...
                                <MessageSquare className={`w-4 h-4 ${
                                  flag.type === 'question' ? 'text-orange-800' : 'text-orange-700'
                                }`} />
                              </div>
                            </div>
                          ...
                          />
                            ...
                            <div className="col-span-2">
                              <div className="flex items-center gap-2">
                                <div className="relative">
                                  <div className="w-8 h-8 bg-gradient-to-br from-orange-400 to-orange-600 rounded-full flex items-center justify-center shadow-md border border-orange-300">
                                    <span className="text-xs font-bold text-white">
                                      {flag.author.charAt(0).toUpperCase()}
                                    </span>
                                  </div>
                                ...
                                <div>
                                  <p className="text-xs font-semibold text-orange-900">{flag.author}</p>
                                  <p className="text-xs text-orange-600">{flag.authorEmail}</p>
                                </div>
                              </div>
                            </div>
                            ...
                            {/* Category */}
                            <div className="col-span-1 flex items-center">
                              <Badge className={`text-xs font-semibold px-2 py-1 rounded-full ${getCategoryBadge(flag.category)}`}>
                                {flag.category}
                              </Badge>
                            </div>
                            ...
                            <div className="col-span-1 flex items-center">
                              <div className="text-center">
                                ...
                                <p className="text-xs text-orange-700 font-semibold">{formatDate(flag.reportedAt)}</p>
                              </div>
                            </div>

                            {/* Actions */}
                            <div className="col-span-2 flex items-center gap-2">
                              <Link href={flag.originalUrl}>
                                <Button 
                                  size="sm" 
                                  variant="ghost" 
                  ...
                  {totalPages > 1 && (
                    <div className="flex items-center justify-between p-4 border-t-2 border-orange-200 bg-gradient-to-r from-orange-50 to-orange-100">
                      <p className="text-xs font-semibold text-orange-800">
                        Showing <span className="font-bold text-orange-600">{((currentPage - 1) * itemsPerPage) + 1}</span> to{' '}
                        <span className="font-bold text-orange-600">{Math.min(currentPage * itemsPerPage, filteredFlags.length)}</span> of{' '}
                        <span className="font-bold text-orange-600">{filteredFlags.length}</span> reports
                      </p>
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
